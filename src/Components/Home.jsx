...
function Home() {
   ...
   useEffect(() => {
      getPlaces().then(res => {
         var last3 = [];
         for (let i = res.length - 1; i >= res.length - 3; i--) {
            last3.push(res[i]);
         }
         setLast3Elm(last3);
      });
            ...
            <div className="flex flex-col h-full items-center">
               ...
               <div className="flex flex-wrap justify-center my-8">
                  {Last3Elm.map((elm, index) => {
                     let images = JSON.parse(elm.Images)
                     return <div key={index} className="md:flex-1 md:px-3 py-2">
                        <Link to='/'>
                           <img className="h-80 w-full object-cover rounded-xl" src={require(`../Upload_images/${images[0]}`)} />
